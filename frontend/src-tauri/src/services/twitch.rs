...
impl TwitchClient {
    pub fn new(http: Client, auth: TwitchAuth) -> Self {
        // Read overrides at construction so tests that race on env vars are
        // not affected mid-call.
        let helix_base = std::env::var("PIXIIS_TWITCH_HELIX_OVERRIDE")
            .unwrap_or_else(|_| HELIX_BASE.to_string());
        let token_url = std::env::var("PIXIIS_TWITCH_TOKEN_URL_OVERRIDE")
    ...
    pub async fn get_top_streams(&self, game_name: &str) -> AppResult<Vec<TwitchStream>> {
        let client_id = self.read_field(|a| a.client_id.clone());
        ...
        let token = self.read_field(|a| a.access_token.clone());
        if token.is_empty() {
            return Ok(Vec::new());
        }
    ...
    fn read_field<T, F>(&self, f: F) -> T
    where
        F: FnOnce(&TwitchAuth) -> T,
    {
    ...
    fn clear_token(&self) {
        if let Ok(mut g) = self.auth.write() {
    ...
    async fn ensure_token(&self) -> AppResult<()> {
        ...
        self.set_token(body.access_token);
        Ok(())
    }
    ...
    async fn resolve_category(
        ...
        let url = format!("{}/search/categories", self.helix_base);
        let resp = self
            .http
            .get(&url)
    ...
    async fn fetch_streams(
        ...
        let url = format!("{}/streams", self.helix_base);
        let resp = self
            .http
            .get(&url)
            .header("Client-Id", client_id)
