//! YouTube Data API v3 — top trailer match for a game.
//!
//! Mirrors `src/pixiis/services/youtube.py`. In-memory cache keyed by
//! lowercased game name; stores up to 5 results, returns the top one.
...
impl YouTubeClient {
    ...
    pub async fn get_trailer(&self, game_name: &str) -> AppResult<Option<YouTubeTrailer>> {
        ...
        if let Some(hit) = self.cache_get_first(&cache_key) {
            return Ok(Some(hit));
        }
        ...
        let top = trailers.first().cloned();
        if let Ok(mut g) = self.cache.lock() {
            g.insert(cache_key, trailers);
        }
        Ok(top)
    }

    fn cache_get_first(&self, key: &str) -> Option<YouTubeTrailer> {
        let g = self.cache.lock().ok()?;
        g.get(key).and_then(|v| v.first().cloned())
    }
}
