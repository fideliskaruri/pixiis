...
//! through the Tauri `asset:` protocol — we hand back a `String` URL so
//! callers can plug it straight into `<img src>`.
//!
...
//! cache dir — Pane 5 owns that wiring.
...
pub struct ImageLoader {
    ...
    inflight: Mutex<HashMap<String, ()>>, // dedupe: not used yet, reserved
}
...
impl ImageLoader {
    ...
    pub async fn request(&self, url: &str) -> AppResult<PathBuf> {
        ...
        // 1. Memory cache
        if let Some(p) = self.mem.lock().unwrap().get(url).cloned() {
            return Ok(p);
        }
        ...
        if fs::try_exists(&path).await.unwrap_or(false) {
            self.mem.lock().unwrap().put(url.to_string(), path.clone());
            return Ok(path);
        }
    ...
    pub fn as_tauri_url(path: &Path) -> String {
        ...
        // Tauri 2 default: `asset://localhost/<encoded>`. On Windows, when
        // `https://` is configured for the asset protocol (Tauri's default
        // for Windows), this still resolves correctly because the webview's
        // protocol handler handles both.
        if cfg!(windows) {
...
mod tests {
    ...
    async fn downloads_and_caches() {
        ...
        // Second call: served from memory cache (no extra HTTP hit).
    ...
    fn as_tauri_url_format() {
        let path = std::path::Path::new("/tmp/x.png");
