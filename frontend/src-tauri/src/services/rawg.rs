...
impl RawgClient {
    pub fn new(http: Client, api_key: String) -> Self {
        let base_url = std::env::var("PIXIIS_RAWG_BASE_URL_OVERRIDE")
    ...
    pub async fn search_game(&self, name: &str) -> AppResult<RawgGameData> {
        ...
        self.cache_put(cache_key, game.clone());
        Ok(game)
    }

    pub async fn get_game_details(&self, id: u64) -> AppResult<RawgGameData> {
        if self.api_key.is_empty() {
            return Ok(RawgGameData::default());
        }
        ...
        let raw: Value = resp.json().await.map_err(http_err)?;
        let game = parse_game(raw);
        self.cache_put(cache_key, game.clone());
        Ok(game)
    }

    fn cache_get(&self, key: &str) -> Option<RawgGameData> {
        self.cache.lock().ok()?.get(key).cloned()
    }

    fn cache_put(&self, key: String, value: RawgGameData) {
