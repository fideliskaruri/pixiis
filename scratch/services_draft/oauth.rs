...
//! * `GET /callback?…` with query string → finishes immediately (auth-code).
...
//! * `GET /token?…` → finishes.
//!
//! Once finished, the server shuts down on its own.
...
/// Owns one in-flight OAuth flow. `start()` boots the server and returns the
/// bound port (the caller is expected to open the auth URL in a browser
/// pointed at `http://127.0.0.1:<port>/callback`). `wait_for_result()` blocks
/// until the redirect lands or `timeout` elapses.
...
impl OAuthFlow {
    ...
    pub async fn start() -> AppResult<Self> {
        ...
        let state = AppState {
            ...
            shutdown: Arc::new(Mutex::new(Some(shutdown_tx.clone_dummy()))),
        };
        // Note: we keep one shutdown sender on the OAuthFlow side and one on
        // the AppState side. The first to fire wins; the other becomes a no-op
        // because oneshot::Sender::send consumes self.
...
// `oneshot::Sender` does not implement Clone. We need *one* shared sender
// because both the cancel path and the success path want to fire shutdown.
// Wrap it in an Arc<Mutex<Option<…>>> via `clone_dummy` (an extension trait
// helper below) to share take semantics.
trait OneshotShare {
    fn clone_dummy(&self) -> Self;
}
impl OneshotShare for oneshot::Sender<()> {
    fn clone_dummy(&self) -> Self {
        // Sender<()> isn't Clone. We sidestep by creating a fresh, never-fired
        // pair; only one of the original or the dummy will actually drive the
        // shutdown — the other drops silently. Practically: the AppState
        // shutdown sender is the one that fires after a successful callback;
        // OAuthFlow::cancel() races, and whichever fires first wins.
        //
        // This trick avoids a second Arc/Mutex layer that would otherwise be
        // needed in AppState. The cost is one extra unused channel pair.
...
fn finish(state: &AppState, params: HashMap<String, String>) {
    ...
    if let Some(tx) = state.shutdown.lock().unwrap().take() {
        // Spawn so the response can flush before the listener stops.
...
mod tests {
    ...
    async fn callback_with_query_completes_flow() {
        ...
        // Hit /callback with a query param.
    ...
    async fn fragment_bridge_completes_via_token() {
        ...
        // 1. /callback with no query → fragment bridge HTML.
        let body = client
