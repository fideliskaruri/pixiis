...
class Transcriber:
    ...
    def transcribe_buffer(self, buffer: list, model, prompt: str = "", fast: bool = False) -> str:
        ...
        `fast=False` uses higher quality settings (beam_size=5).
        `fast=True` uses lower quality/faster settings (beam_size=3).
        """
        if not buffer:
            return ""
        ...
        try:
            ...
            # Use consistent settings: no context prompting (reduces hallucinations)
            segs, _ = model.transcribe(
                ...
                condition_on_previous_text=False,  # disable to avoid hallucinations
                initial_prompt=None,  # disable context prompting
