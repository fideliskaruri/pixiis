...
//! `Gilrs` here — Pane 8 owns the gamepad runtime. Caller passes the controller
//! index; we use XInput directly because gilrs's force-feedback story on
//! XInput is patchy (effects vary by driver).
...
mod win {
    ...
    pub fn set_state(index: u32, left: u16, right: u16) {
        ...
        // SAFETY: XInputSetState is a thread-safe Win32 call; we pass a valid
        // pointer to a stack-allocated XINPUT_VIBRATION.
        unsafe {
...
///
/// `left` / `right` are 0..=65535. On non-Windows platforms this is a no-op.
